...
from .components import syncronize_movables

from ..client.rpcs import spawn_player

from ..listener import notify_available_server
from ..components import NetworkEntityMap, NetEntity
...
def on_client_connection(resources: Resources, event: ClientConnectedEvent):
    ...
    new_player_addr = event.addr
    new_player_uid = netmap.consume_entity_uid()
    new_player_pos = (0, 0)
    
    player_ent = world.create_entity(*make_policeman(
        new_player_uid,
        new_player_pos,
        new_player_addr == session.game_owner,
        assets
    ))

    print(f"A new player with uid {new_player_uid} has connected!")

    # First we spawn our new player as main player on the new client
    server.call(new_player_addr, spawn_player, new_player_uid, *new_player_pos, True)

    # Now, we notify all other players of this new client's player
    for addr, ent in session.players.items():

        # Notify of our new client's player
        server.call(addr, spawn_player, new_player_uid, *new_player_pos, False)

        # If entity exists - we will also notify our new client, of other clients' players
        if ent is not None:
            uid = world.get_component(ent, NetEntity).get_uid()
            server.call(new_player_addr, spawn_player, uid, *(0, 0), False)

    session.players[new_player_addr] = player_ent

def on_client_disconnection(resources: Resources, event: ClientDisconnectedEvent):
    session = resources[ServerSession]
    world = resources[WorldECS]

    print(f"Client {event.addr} has been disconnected")
...
class ServerSessionPlugin(Plugin):
    def build(self, app):
